                ...
                if (event === 'SIGNED_IN' && session?.user) {
                    setTimeout(async () => {
                        try {
                            ...
                            const userRole = metadataRole || sessionRole || 'merchant';

                            console.log('🔍 Role Detection:');
                            console.log('  user_metadata.role:', metadataRole);
                            console.log('  sessionStorage[selected_role]:', sessionRole);
                            console.log('  Final userRole:', userRole);
                            console.log('  Full user_metadata:', JSON.stringify(session.user.user_metadata, null, 2));

                            // STEP 3: Create role entry
                            console.log('📌 About to insert role into user_roles table');
                            console.log('  user_id:', session.user.id);
                            console.log('  role:', userRole);
                            console.log('  role type:', typeof userRole);

                            const { error: roleError, data: roleInsertData } = await supabase
                                .from('user_roles')
                                .insert({
                            ...
                            console.log('  Full roleError details:', JSON.stringify(roleError, null, 2));
