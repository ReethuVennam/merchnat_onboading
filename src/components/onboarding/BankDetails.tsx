...
//             const filePath = `${user_id}_${safePhone}/cancelled-cheques/${fileName}`;
...
// }) => {
//     const { toast } = useToast();
//     const { saveBankDetails, merchantProfile: dbMerchantProfile } = useMerchantData();
//     // Use prop-passed merchant profile if available (for distributor onboarding), otherwise use DB fetch
...
//             if (!merchantProfile?.id) {
//                 console.error('❌ Merchant profile not found:', { propMerchantProfile, dbMerchantProfile });
//                 throw new Error("Merchant profile not found. Please ensure merchant is properly created.");
//             }
...
//             console.error('❌ Merchant profile not available:', merchantProfile);
//             toast({
//                 variant: "destructive",
//                 title: "Error",
//                 description: "Merchant profile not found",
...
//         try {
//             // In distributor flow, we skip the saveBankDetails call and just update parent state
//             // The final submit handler will save everything to the database
//             let shouldCallSaveBankDetails = true;
            
//             if (propMerchantProfile && propMerchantProfile.id) {
//                 // Distributor mode - propMerchantProfile is being used, don't call saveBankDetails
//                 shouldCallSaveBankDetails = false;
//             }

//             // Save bank details to database only in normal (non-distributor) flow
//             if (shouldCallSaveBankDetails) {
//                 await saveBankDetails({
//                     account_number: formData.accountNumber,
//                     ifsc_code: formData.ifscCode,
//                     bank_name: formData.bankName,
